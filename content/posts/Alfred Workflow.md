...
![Alfred Workflow](/Users/Zac/screenshots/Alfred Workflow.jpg)

Alfred has a development environment that might remind you of [Yahoo Pipes](https://en.wikipedia.org/wiki/Yahoo!_Pipes), [Microsoft's VPL](https://learn.microsoft.com/en-us/previous-versions/microsoft-robotics/bb483088(v=msdn.10)?redirectedfrom=MSDN) or even [IFTTT](https://ifttt.com/explore/welcome_to_ifttt). Alfred's Workflows are different than many visual programming languages in that the routes in the Workflow are primarily to represent flow and you double-click the icons to configure them or to add scripts and data.
...
You can see that you are able to set the keywords used, tell Alfred if it requires an argument and also provide the user with some feedback as to what the Workflow does. The connections between each element in the workflow is dragged from the left side of one object to the right side of the next element in the Workflow.
...
> example of Workflow running

As the user filters the list of data that is provided in the List Filter the Title and Subtitle are displayed to give them more information about the items they are looking at. The rest of your Workflow can't currently access those items and will only be able to use the data stored in the Args. 

This posed a bit of a problem for this example since the spells in the `.csv` file had 18 columns of data in them. Obviously I wouldn't be showing them all in this example but I wanted to have more than one to display. Happily it was easy to use [Easy CSV](https://vdt-labs.com/easy-csv-editor/) to merge some of the column together and then delete the ones I didn't want.  In addition, Easy CSV was able to put a delimiter between the data from each column. This will be used later to break the data into individual arguments. As well, since the Title won't propogate into the rest of the Workflow, I added the name of each spell to the Args column. 
...
Now we have the data we just need to do something with it.
...
While I was initially working on this example, I tried using the Notification and Dialog Output objects to display the data. Those both worked quite well but didn't allow me to format the results to make them look good. There is also the problem that many of the spells contain far more data that a notification can display on the Mac. I searched for some options, and even looked at using an [AppleScript created window](https://forum.latenightsw.com/t/dialog-toolkit-plus-extended-date-pickers/4154), but in the end I decided that what I would do was create an HTML page and then replace the placeholders [^6] in the file with the data that is being generated in Alfred.

Then I started thinking [^7]. What if I could render the HTML and then turn that into an image? This took me down quite the interesting rabbit-hole and in the end I found two command line applications that could do it. [Webkit2PNG](https://paulhammond.org/webkit2png) is a Python based tool that uses the Webkit rendering engine to turn a URL into a series of `.png` images. It looked useful but the more interesting option was  [pageres-cli](https://github.com/sindresorhus/pageres-cli) by [Sindre Sorhus](https://sindresorhus.com/) [^8] which has far more options available to limit the area it returns as well as limit the rendering to a specific selector. 

This then is the plan. I would create and open an HTML page and, if the user presses the Shift key when they select the spell, create a bitmap image of the HTML and show that to them instead. 

## The script

So we have the data split into individual arguments and we know how the data is going to be displayed. The first thing to do is to take the arguments passed from the Split Args object and use them in a shell script [^9]. Alfred has two options if you want to run a script in a Workflow. The Run Script object will let you run Python, Ruby, AppleScript, JavaScript, Perl, Swift and bash or zsh shell script. You can also get the Run Script object to run an external script. 

> first script image

### A second digression

During all this mucking about I discovered that the _sed_ utility that ships with the Mac is a total pain-in-the-ass to use. There is probably a [very complicated explanation](https://riptutorial.com/sed/topic/9436/bsd-macos-sed-vs--gnu-sed-vs--the-posix-sed-specification) of the difference between the two and I couldn't really find a simple one. On the Mac, and some Linux distros, the _sed_ command will generate errors that are clearly based on the command not parsing the data you send to it properly. It is an old command that is kept on some Unix flavours primarily to keep POSIX compatibility but it won't work correctly, even with properly formatted commands, on the Mac. I spent a few hours trying to figure out why my _sed_ commands wouldn't work onl;y to have them work flawlessly with _gsed_
...
There is probably a quicker way to write this as a shell script but this works well enough for what I need. This first script will run a zsh shell script to move to the project directory, copy the original HTML file and then replace the placeholders using _gsed_ [^10]. 
...
[^3]: CSS stands for _Conquest Spell Search_ but only later did I realise that it also stands for, well, CSS. 
...
[^6]: There are better ways to do this text replacement but they involve using JavaScript, a templating system and a `.json` file. If anyone is interested I can write up an article describing that. 

[^7]: Always a bad idea. 

[^8]: He must not sleep. He writes Mac apps, CLI tools and also appears to write an enormous amount of Open Source JavaScript code. 

[^9]: Actually I had to make the HTML and CSS first but that isn't very interesting. 

[^10]: Installed using [Homebrew](https://brew.sh/). 